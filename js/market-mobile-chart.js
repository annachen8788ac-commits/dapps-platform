...
function showDetail(p){if(gesture)return;longPress=true;gesture='inspect';chart.classList.add('mobile-inspect');mouse('mousemove',p.x,p.y,0)}
...
findCanvas();
chart.addEventListener('touchstart',e=>{findCanvas();if(!canvas)return;clearHover();if(e.touches.length===2){cancelLongPress();hideDetail();gesture='pinch';startDistance=distance(e.touches[0],e.touches[1]);lastCenter=center(e.touches[0],e.touches[1]);dragging=false}else if(e.touches.length===1){const p=point(e.touches[0]);startPoint=p;lastCenter=p;dragging=false;gesture=null;cancelLongPress();longPressTimer=setTimeout(()=>showDetail(p),500)}},{passive:true});
chart.addEventListener('touchmove',e=>{if(!canvas)return;if(e.touches.length===2){cancelLongPress();hideDetail();gesture='pinch';const d=distance(e.touches[0],e.touches[1]),c=center(e.touches[0],e.touches[1]);if(startDistance>0&&Math.abs(d-startDistance)>3){wheelAt(c.x,c.y,d>startDistance?-90:90);startDistance=d}lastCenter=c;return}if(e.touches.length!==1||!startPoint)return;const p=point(e.touches[0]),dx=p.x-startPoint.x,dy=p.y-startPoint.y,ax=Math.abs(dx),ay=Math.abs(dy);if(longPress||gesture==='inspect'){mouse('mousemove',p.x,p.y,0);lastCenter=p;return}if(!gesture&&Math.max(ax,ay)>10){cancelLongPress();gesture=ax>ay*1.25?'horizontal':'vertical'}if(gesture==='vertical'){cancelLongPress();hideDetail();return}if(gesture==='horizontal'){if(!dragging){dragging=true;mouse('mousedown',startPoint.x,startPoint.y)}mouse('mousemove',p.x,p.y);lastCenter=p}},{passive:true});
chart.addEventListener('touchend',e=>{cancelLongPress();if(dragging&&lastCenter)mouse('mouseup',lastCenter.x,lastCenter.y);hideDetail();if(e.touches.length<2)startDistance=0;if(e.touches.length===0){dragging=false;lastCenter=null;startPoint=null;gesture=null}},{passive:true});
chart.addEventListener('touchcancel',()=>{cancelLongPress();if(canvas&&dragging&&lastCenter)mouse('mouseup',lastCenter.x,lastCenter.y);hideDetail();dragging=false;startDistance=0;lastCenter=null;startPoint=null;gesture=null},{passive:true});
