...
chart.addEventListener('touchstart',e=>{findCanvas();if(!canvas)return;clearHover();if(e.touches.length===2){e.preventDefault();cancelLongPress();hideDetail();startDistance=distance(e.touches[0],e.touches[1]);startCenter=center(e.touches[0],e.touches[1]);lastCenter=startCenter;dragging=false}else if(e.touches.length===1){e.preventDefault();const p=point(e.touches[0]);startPoint=p;lastCenter=p;dragging=false;cancelLongPress();longPressTimer=setTimeout(()=>showDetail(p),500)}},{passive:false});
