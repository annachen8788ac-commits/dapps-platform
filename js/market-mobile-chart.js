...
function hideDetail(){if(detailTimer){clearTimeout(detailTimer);detailTimer=null}chart.classList.remove('mobile-inspect');const tip=chart.querySelector('.pro-chart-tooltip');if(tip)tip.classList.remove('show');clearHover()}
function showDetail(p){hideDetail();chart.classList.add('mobile-inspect');mouse('mousemove',p.x,p.y,0);detailTimer=setTimeout(hideDetail,2000)}
findCanvas();
chart.addEventListener('touchstart',e=>{findCanvas();if(!canvas)return;if(e.touches.length===2){hideDetail();gesture='pinch';startDistance=distance(e.touches[0],e.touches[1]);lastCenter=center(e.touches[0],e.touches[1]);dragging=false}else if(e.touches.length===1){const p=point(e.touches[0]);startPoint=p;lastCenter=p;dragging=false;gesture=null}},{passive:true});
chart.addEventListener('touchmove',e=>{if(!canvas)return;if(e.touches.length===2){hideDetail();gesture='pinch';const d=distance(e.touches[0],e.touches[1]),c=center(e.touches[0],e.touches[1]);if(startDistance>0&&Math.abs(d-startDistance)>3){wheelAt(c.x,c.y,d>startDistance?-90:90);startDistance=d}lastCenter=c;return}if(e.touches.length!==1||!startPoint)return;const p=point(e.touches[0]),dx=p.x-startPoint.x,dy=p.y-startPoint.y,ax=Math.abs(dx),ay=Math.abs(dy);if(!gesture&&Math.max(ax,ay)>10){gesture=ax>ay*1.25?'horizontal':'vertical';hideDetail()}if(gesture==='vertical')return;if(gesture==='horizontal'){if(!dragging){dragging=true;mouse('mousedown',startPoint.x,startPoint.y)}mouse('mousemove',p.x,p.y);lastCenter=p}},{passive:true});
